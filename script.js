...
//                name.includes('george') || name.includes('guy') || name.includes('james') || name.includes('richard');
//     };
...
//         btn.onclick = () => selectQuizOption(btn, optText, targetMeaning, currentW.word);
...
//         quizErrorsList.push({ word: currentWord, userChoice: selectedText, correctMeaning: correctMeaning });
...
//         const armText = item.arm || item.armenian;
//         cardsData.push({ id: index, text: item.word, type: 'en', pairText: armText });
//         cardsData.push({ id: index, text: armText, type: 'arm', pairText: item.word });
//     });
...
//         if (questionContainer) questionContainer.innerHTML = `🎉 Scramble Completed! Score: ${scrambleScore}`;
...
//     if (questionContainer) questionContainer.innerText = shuffleText(scrambleCurrentWord.word.toUpperCase());
...
//             feedbackMsg.innerText = `❌ Wrong! Answer: "${scrambleCurrentWord.word}"`;
//         }
//     }
